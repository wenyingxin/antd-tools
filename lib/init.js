...
function addConfigHooks(cfg, projectDir) {
  ...
  cfg.scripts = Object.assign(cfg.scripts, {
    dist: 'rubix-tools run dist',
    compile: 'rubix-tools run compile',
    clean: 'rubix-tools run clean',
    start: 'rubix-tools run start',
    site: 'rubix-tools run site',
    deploy: 'rubix-tools run update-self && rubix-tools run deploy',
    'just-deploy': 'rubix-tools run just-deploy',
    pub: 'rubix-tools run update-self && rubix-tools run pub',
  });
  ...
  cfg.scripts.prepublish = 'rubix-tools run guard';
...
function init() {
  ...
  if (!testMode) {
    ...
    if (!npmArgs || !npmArgs.some(arg => /^rubix-tools(@\d+\.\d+.\d+)?$/.test(arg))) {
      return;
    }
  }
